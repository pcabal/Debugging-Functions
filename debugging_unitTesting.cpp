...
/** TEST 1 -- IS THE TREE CORRECTLY SET UP AND POPULATED **/
    /* Compute the height of the tree, should be 11 including the root or 10 excluding the root*/
    int BTree :: maxDepth(BTreeNode * node)
    {
    if (node==NULL)
        return 0;
    else
...
int main()
{
    ...
    char * genome_file_name = "/Users/Perseus/Documents/C++/in-class-exercise/input1.txt";
    ...
    /** TEST 1 -- IS THE TREE CORRECTLY SET UP AND POPULATED **/
    /* Compute the height of the tree, should be 11 including the root or 10 excluding the root*/
    if (b->maxDepth(b->Root->Lchild) == 10)
    {
        cout << "Depth of the tree: " << b->maxDepth(b->Root->Lchild) << endl;
        cout << "This tree has the right height" << endl;
    }
    else
        cout << "The height of your tree should be ten, something is wrong with how the tree is set up" << endl;


    vector<string> reads;
    //char * reads_file_name="input2.txt";       //make certain to place this file in the correct folder. Do not change path.
    char * reads_file_name = "/Users/Perseus/Documents/C++/in-class-exercise/input2.txt";
